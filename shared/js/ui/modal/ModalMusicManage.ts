export namespace Modals {
    ...
    function permission_controller(event_registry: events.Registry<events.modal.music_manage>, bot: MusicClientEntry, client: ConnectionHandler) {
            ...
            event_registry.on("query_group_permissions", event => {
                client.permissions.find_permission(event.permission_name).then(result => {
                    ...
                    for(const e of result) {
                        ...
                        const group = client.groups.serverGroup((e as permissions.find.ServerGroup).group_id);
                        if(!group) continue;
    ...
    function dummy_controller(event_registry: events.Registry<events.modal.music_manage>) {
        /* settings */
        {
            event_registry.on("query_bot_status", event => {
                setTimeout(() => {
    ...
    function build_modal(event_registry: events.Registry<events.modal.music_manage>) : JQuery<HTMLElement> {
    ...
    function build_settings_container(event_registry: events.Registry<events.modal.music_manage>, tag: JQuery<HTMLElement>) {
        ...
        /* music bot settings */
        {
            ...
            /* country flag */
            {
                ...
                const update_country_code = input => {
                    ...
                    flag.attr("title", i18n.country_name(input, tr("Unknown country")));
                };
    ...
    function build_permission_container(event_registry: events.Registry<events.modal.music_manage>, tag: JQuery<HTMLElement>) {
